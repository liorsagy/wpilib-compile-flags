...
if len(argv) > 1 and exists(join(argv[1], "vendordeps")):
    argv[1] = join(argv[1], "vendordeps")
    for dep in listdir(argv[1]):
        try:
            vendordeps = loads(open(join(argv[1], dep), "r").read())

            for d in vendordeps["cppDependencies"]:
                dependencies.append(
                    Dependency(
                        url=(f"{vendordeps['mavenUrls'][0].removesuffix('/')}/{d['groupId'].replace('.', '/')}" if d["version"] != "wpilib" else "https://frcmaven.wpi.edu/ui/api/v1/download?repoKey=release&path=edu/wpi/first/wpilibNewCommands"),
